...
{{-- ============================================
     WRAPPER UTAMA
     x-data di sini = parent scope
     semua child bisa dispatch event ke sini
============================================ --}}
<div class="max-w-6xl mx-auto px-4 py-8"
     ...
     x-data="{
         ...
         {{-- Harga kamar per malam dari PHP --}}
         pricePerNight: {{ $room->price_per_night }},

         {{-- Object untuk menyimpan semua item FnB yang dipilih --}}
         {{-- Format: { id: { price, name, qty } } --}}
         fnbItems: {},

         {{-- Hitung jumlah malam dari selisih tanggal --}}
         get nights() {
         ...
         {{-- Total harga kamar = harga/malam × jumlah malam --}}
         get roomTotal() {
             return this.nights * this.pricePerNight
         },

         {{-- Total harga semua FnB yang dipilih --}}
         get fnbTotal() {
             let total = 0
         ...
         {{-- Tambah / update / hapus item FnB --}}
         {{-- Dipanggil saat menerima event 'fnb-update' dari child --}}
         setFnb(id, price, name, qty) {
             if (qty <= 0) {
                 {{-- Qty 0 = hapus dari object --}}
                 delete this.fnbItems[id]
                 {{-- Trigger Alpine reactivity dengan replace object --}}
     ...
     {{-- Listen custom event dari child item FnB --}}
     {{-- .window = listen di level window agar bisa tangkap dari nested child --}}
     @fnb-update.window="setFnb($event.detail.id, $event.detail.price, $event.detail.name, $event.detail.qty)">

    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">

        {{-- ==========================================
             KOLOM KIRI: Info Kamar + Form Booking
        ========================================== --}}
        <div class="lg:col-span-2 space-y-6">
            ...
            {{-- ==========================================
                 FORM BOOKING
                 id="booking-form" agar tombol submit
                 di sidebar bisa trigger form ini
            ========================================== --}}
            <form action="{{ route('public.booking.store', $room) }}"
                  method="POST"
                  id="booking-form">
                @csrf
                ...
                <div class="bg-white rounded-2xl border border-gray-100 p-6 mb-5">
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        ...
                        <div>
                            ...
                            {{-- :min="checkIn" agar check-out tidak bisa sebelum check-in --}}
                            <input type="date"
                                   name="check_out"
                                   x-model="checkOut"
                                   :min="checkIn"
                ...
                {{-- ==========================================
                     ...
                     Komunikasi ke parent via $dispatch
                ========================================== --}}
                @if($fnbItems->count() > 0)
                <div class="bg-white rounded-2xl border border-gray-100 p-6">
                    ...
                    @foreach($fnbItems as $categoryId => $items)
                        ...
                        <div class="mb-6">
                            ...
                            <div class="space-y-2">
                                ...
                                {{-- ==========================================
                                     ITEM FNB — x-data qty sendiri
                                     Tidak mengakses parent langsung,
                                     tapi dispatch event ke window
                                ========================================== --}}
                                <div class="flex items-center gap-3 p-3 border border-gray-100 rounded-xl
                                            hover:border-green-200 transition"
                                     x-data="{ qty: 0 }"
                                     {{-- Highlight border jika qty > 0 --}}
                                     :class="qty > 0 ? 'border-green-300 bg-green-50' : 'border-gray-100'">
                                    ...
                                    {{-- ==========================================
                                         QTY CONTROL
                                         - qty disimpan di x-data lokal item ini
                                         - saat berubah, dispatch event ke window
                                         - parent menangkap via @fnb-update.window
                                    ========================================== --}}
                                    <div class="flex items-center gap-2 flex-shrink-0">
                                        ...
                                        <button type="button"
                                                ...
                                                :class="qty === 0
                                                    ...
                                                    : 'text-gray-600 hover:bg-red-50 hover:border-red-200 hover:text-red-500'">
                                            <i class="ti ti-minus text-xs"></i>
                                        </button>

                                        {{-- Tampilan qty --}}
                                        <span class="w-8 text-center text-sm font-bold transition"
                                              :class="qty > 0 ? 'text-green-700' : 'text-gray-400'"
                                              x-text="qty">
                                        </span>

                                        {{-- Hidden input — name kosong jika qty 0 agar tidak ikut submit --}}
                                        <input type="hidden"
        ...
        <div class="lg:col-span-1">
            <div class="bg-white rounded-2xl border border-gray-100 p-5 sticky top-24">
                ...
                <div class="space-y-2 mb-4">
                    ...
                    <div class="flex items-center justify-between text-sm">
                        ...
                        <span class="font-medium text-gray-800"
                              x-text="formatRupiah(roomTotal)">
                        </span>
                    </div>

                    {{-- FnB yang dipilih — muncul dinamis --}}
                    {{-- x-for melakukan loop pada object fnbItems --}}
                    <template x-for="(item, id) in fnbItems" :key="id">
                        <div class="flex items-center justify-between text-sm">
                            <span class="text-gray-500 truncate max-w-32"
                                  x-text="item.name + ' ×' + item.qty">
                            </span>
                ...
                {{-- Tombol submit — trigger form booking --}}
                <button type="submit"
                        form="booking-form"
