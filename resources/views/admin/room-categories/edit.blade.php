...
@section('title', 'Edit Kategori — ' . $roomCategory->name)
...
<div class="flex items-center gap-2 text-sm text-gray-400 mb-6">
    ...
    <span class="text-gray-600 font-medium">Edit {{ $roomCategory->name }}</span>
</div>
...
<div class="max-w-xl">
    <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        ...
        <div class="px-6 py-5 border-b border-gray-100">
            <h2 class="font-bold text-gray-800">Edit Kategori Kamar</h2>
            <p class="text-sm text-gray-400 mt-0.5">
                Terdapat {{ $roomCategory->rooms_count ?? $roomCategory->rooms()->count() }} kamar di kategori ini
            </p>
        </div>

        <form action="{{ route('admin.room-categories.update', $roomCategory) }}"
              method="POST"
              class="p-6 space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="description"
                          ...
                          class="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-xl bg-gray-50
                                 focus:outline-none focus:border-green-500 focus:ring-2 focus:ring-green-100
                                 resize-none">{{ old('description', $roomCategory->description) }}</textarea>
            </div>
