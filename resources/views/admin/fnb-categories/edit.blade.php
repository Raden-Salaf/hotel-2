...
@section('title', 'Edit Kategori — ' . $fnbCategory->name)
...
<div class="flex items-center gap-2 text-sm text-gray-400 mb-6">
    ...
    <span class="text-gray-600 font-medium">Edit {{ $fnbCategory->name }}</span>
</div>
...
<div class="max-w-lg">
    <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        ...
        <div class="px-6 py-5 border-b border-gray-100">
            <h2 class="font-bold text-gray-800">Edit Kategori F&B</h2>
            <p class="text-sm text-gray-400 mt-0.5">
                Terdapat
                {{ $fnbCategory->items()->count() }} menu di kategori ini
            </p>
        </div>

        <form action="{{ route('admin.fnb-categories.update', $fnbCategory) }}"
              method="POST"
              class="p-6 space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text"
                       name="name"
                       value="{{ old('name', $fnbCategory->name) }}"
            ...
            <div>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0
                         ...
                         id="icon-preview">
                        {{ old('icon', $fnbCategory->icon ?? '🍽️') }}
                    </div>
                    <input type="text"
                           name="icon"
                           value="{{ old('icon', $fnbCategory->icon) }}"
