...
@section('title', 'Edit Item Laundry — ' . $laundry->name)
...
@section('content')
    ...
    <div class="flex items-center gap-2 text-sm text-gray-400 mb-6">
        ...
        <span class="text-gray-600 font-medium">Edit {{ $laundry->name }}</span>
    </div>
    ...
    <div class="max-w-lg">
        <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden">
            ...
            <div class="px-6 py-5 border-b border-gray-100">
                <h2 class="font-bold text-gray-800">Edit Item Laundry</h2>
            </div>

            <form action="{{ route('admin.laundry.update', $laundry) }}" method="POST" class="p-6 space-y-5">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <textarea name="description" rows="2"
                        class="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-xl
                                 bg-gray-50 focus:outline-none focus:border-green-500
                                 focus:ring-2 focus:ring-green-100 resize-none">{{ old('description', $laundry->description) }}</textarea>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="price" value="{{ old('price', $laundry->price) }}"
                            class="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-xl
                                  bg-gray-50 focus:outline-none focus:border-green-500
                                  focus:ring-2 focus:ring-green-100">
                    </div>
                    <div>
                        ...
                        <select name="unit"
                            class="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-xl
                                   bg-gray-50 focus:outline-none focus:border-green-500
                                   focus:ring-2 focus:ring-green-100">
                            <option value="pcs" {{ old('unit', $laundry->unit) === 'pcs' ? 'selected' : '' }}>
                                Per Pcs
                            </option>
                            <option value="kg" {{ old('unit', $laundry->unit) === 'kg' ? 'selected' : '' }}>
                                Per Kg
                            </option>
                        </select>
                    </div>
                ...
                <div>
                    ...
                    <div class="flex items-center gap-3">
                        <div class="w-12 h-12 rounded-xl flex items-center justify-center
                            ...
                            id="icon-preview-edit">
                            {{ old('icon', $laundry->icon ?? '👕') }}
                        </div>
                        <input type="text" name="icon" value="{{ old('icon', $laundry->icon) }}" maxlength="10"
                            class="flex-1 px-3 py-2.5 text-sm border border-gray-200 rounded-xl
                                  bg-gray-50 focus:outline-none focus:border-green-500
                                  focus:ring-2 focus:ring-green-100"
                            oninput="document.getElementById('icon-preview-edit').textContent = this.value || '👕'">
                    </div>
                        ...
                        @foreach (['👕', '👖', '🧥', '👗', '👔', '🧤', '🧦', '👙', '🩱', '🩲', '🩳', '👟'] as $emoji)
                            <button type="button"
                                ...
                                onclick="
                                    ...
                                    document.getElementById('icon-preview-edit').textContent = '{{ $emoji }}';
                                ">
                                {{ $emoji }}
                            </button>
                        @endforeach
                ...
                <div>
                    ...
                    <label
                        ...
                        x-data="{ on: {{ old('is_available', $laundry->is_available) ? 'true' : 'false' }} }" :class="on ? 'border-green-500 bg-green-50' : 'border-gray-200'">
