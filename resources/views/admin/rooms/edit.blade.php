...
@section('title', 'Edit Kamar — ' . $room->name)
...
@section('content')
    ...
    <div class="flex items-center gap-2 text-sm text-gray-400 mb-6">
        ...
        <span class="text-gray-600 font-medium">Edit {{ $room->name }}</span>
    </div>
    ...
    <div class="max-w-3xl">
        <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden">
            ...
            <div class="px-6 py-5 border-b border-gray-100">
                <h2 class="font-bold text-gray-800">Edit Kamar</h2>
                <p class="text-sm text-gray-400 mt-0.5">No. Kamar {{ $room->room_number }}</p>
            </div>

            {{-- Form edit — method PUT karena ini update, bukan create --}}
            {{-- HTML form tidak support PUT, jadi pakai @method('PUT') --}}
            <form action="{{ route('admin.rooms.update', $room) }}" method="POST" enctype="multipart/form-data"
                class="p-6 space-y-5">
                @csrf
                @method('PUT') {{-- Ini penting! Memberitahu Laravel ini adalah request UPDATE --}}
                    ...
                    <div>
                        ...
                        <select name="room_category_id"
                            ...
                            @foreach ($categories as $cat)
                                <option value="{{ $cat->id }}" {{-- old() untuk saat validasi gagal, $room->room_category_id untuk nilai awal --}}
                                    {{ old('room_category_id', $room->room_category_id) == $cat->id ? 'selected' : '' }}>
                                    {{ $cat->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div>
                    <label class="block text-xs font-semibold text-gray-600 mb-1.5">Nama Kamar <span
                ...
                <div>
                    ...
                    <textarea name="description" rows="3"
                        class="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-xl bg-gray-50 focus:outline-none focus:border-green-500 focus:ring-2 focus:ring-green-100 resize-none">{{ old('description', $room->description) }}</textarea>
                </div>

                <div class="grid grid-cols-3 gap-4">
                    <div>
                        <label class="block text-xs font-semibold text-gray-600 mb-1.5">Harga/Malam (Rp) <span
                                class="text-red-400">*</span></label>
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-gray-600 mb-1.5">Kapasitas <span
                                class="text-red-400">*</span></label>
                ...
                <div x-data="{ selected: '{{ old('status', $room->status) }}' }" class="flex items-center gap-3">
                    <label class="block text-xs font-semibold text-gray-600 mb-1.5">
                ...
                <div>
                    ...
                    @php
                        ...
                        $selectedFasilitas = old('facilities', $room->facilities ?? []);
                    @endphp
                    <div class="flex flex-wrap gap-2">
                        @foreach ($fasilitasList as $fasilitas)
                            <label
                                class="flex items-center gap-1.5 px-3 py-1.5 border rounded-lg cursor-pointer text-xs
                                      {{ in_array($fasilitas, $selectedFasilitas) ? 'border-green-500 bg-green-50 text-green-700' : 'border-gray-200 text-gray-600' }}">
